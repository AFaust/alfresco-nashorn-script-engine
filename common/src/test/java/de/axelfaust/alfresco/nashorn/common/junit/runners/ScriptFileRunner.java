 ...
 */
package de.axelfaust.alfresco.nashorn.common.junit.runners;
...
import de.axelfaust.alfresco.nashorn.common.junit.interop.InvokeScriptFunction;
import de.axelfaust.alfresco.nashorn.common.junit.interop.JUnitAfterAwareScript;
import de.axelfaust.alfresco.nashorn.common.junit.interop.JUnitAfterScriptAwareScript;
import de.axelfaust.alfresco.nashorn.common.junit.interop.JUnitBeforeAwareScript;
import de.axelfaust.alfresco.nashorn.common.junit.interop.JUnitBeforeScriptAwareScript;
import de.axelfaust.alfresco.nashorn.common.junit.interop.JUnitTestCaseAwareScript;
import de.axelfaust.alfresco.nashorn.common.junit.interop.JUnitTestScript;
import de.axelfaust.alfresco.nashorn.common.junit.interop.RunScriptLifecycleMethods;
import de.axelfaust.alfresco.nashorn.common.junit.interop.ScriptFunction;
...
public class ScriptFileRunner extends ParentRunner<ScriptFunction>
{
     ...
     * without any Java-based preparation requirements.
     *
     * @author Axel Faust
     */
    @Ignore
    public static class DummyScriptTestCase extends TestCase
    {
        // NO-OP
    }
    ...
    static
    {
        ...
        NASHORN_ENGINE = scriptEngineFactory.getScriptEngine("-scripting=false", "--debug-lines=true", "--debug-locals=true");
    }
    ...
    protected static Class<? extends TestCase> determineTestCaseClass(final String scriptFile) throws InitializationError
    {
        ...
        final URL resource = ScriptFileRunner.class.getClassLoader().getResource(scriptFile);
        if (resource == null)
        {
            ...
            throws ScriptException
    {
        ...
        {
            scriptContext.setAttribute(ScriptEngine.FILENAME, resource, ScriptContext.ENGINE_SCOPE);
    ...
    protected ScriptFileRunner(final String scriptFile) throws InitializationError
    {
    ...
    {
        final List<ScriptFunction> children = new ArrayList<>();
    ...
    protected final ConcurrentHashMap<ScriptFunction, Description> functionDescriptions = new ConcurrentHashMap<>();
    ...
    protected Statement withBeforeClasses(final Statement statement)
    {
        ...
        resultStatement = scriptBefores.isEmpty() ? resultStatement
                : new RunScriptLifecycleMethods(resultStatement, false, scriptBefores, null, this.scriptFile, NASHORN_ENGINE);
        ...
        if (((Invocable) NASHORN_ENGINE).getInterface(this.scriptTestObject, JUnitBeforeScriptAwareScript.class) != null)
        {
            ...
            try
            {
                target = new ReflectiveCallable()
                {
                    ...
                    {
                        return ScriptFileRunner.this.createTest();
                    }
                }.run();
                scriptTarget = new ReflectiveCallable()
                {
                    ...
                    protected Object runReflectiveCall() throws Throwable
                    {
                        return ScriptFileRunner.this.createScriptTest(target);
                    }
                }.run();
            }
    ...
    protected Statement withAfterClasses(final Statement statement)
    {
        ...
        resultStatement = scriptAfters.isEmpty() ? resultStatement
                : new RunScriptLifecycleMethods(resultStatement, true, scriptAfters, null, this.scriptFile, NASHORN_ENGINE);
    ...
    protected Statement methodBlock(final ScriptFunction scriptFunction)
    {
        ...
        try
        {
            ...
            scriptTest = new ReflectiveCallable()
            {
                ...
                protected Object runReflectiveCall() throws Throwable
                {
                    return ScriptFileRunner.this.createScriptTest(test);
                }
            }.run();
        }
        catch (final Throwable e)
    ...
    {
        Statement resultStatement = statement;
        final JUnitAfterAwareScript beforeAwareScript = ((Invocable) NASHORN_ENGINE).getInterface(scriptTarget,
                JUnitAfterAwareScript.class);
        if (beforeAwareScript != null)
        {
    ...
    protected Object createScriptTest(final Object test) throws Exception
    {
        ...
        final URL resource = ScriptFileRunner.class.getClassLoader().getResource(this.scriptFile);
