...
{
    'use strict';
    var noSuchPropertyStackKey, proxyEnabledKey, adapteeFunctionNames, Locale, boolGetterNames, getterNames, setterNames;

    noSuchPropertyStackKey = '--proxy-no-such-property-stack';
    proxyEnabledKey = '--proxy-support-enabled';

    adapteeFunctionNames = [ '__get__', '__put__', '__has__', '__delete__', '__getIds__', '__getKeys__', '__getValues__', '__call__',
            '__new__' ];
    ...
    return declare([], {
        ...
        __getIds__ : function ProxySupport__getIds__()
        {
            ...
            result = [];
            // TODO Check (simulated) getters and non-function properties
            for (name in this)
            {
                result.push(name);
            }
