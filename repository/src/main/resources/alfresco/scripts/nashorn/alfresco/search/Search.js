...
define([ 'require', '_base/declare', 'alfresco/foundation/NodeService', 'alfresco/foundation/PermissionService', '_base/logger',
        'nashorn!Java' ], function alfresco_search_Search(require, declare, NodeService, PermissionService, logger, Java)
{
    'use strict';
    ...
    /**
     * This module provides the same abstraction for querying / looking up nodes in the Alfresco repository as the Rhino-based class Search
     * does. Functions that behave differently than the Rhino-based implementation are documentated accordingly.
     * 
    ...
    module = {
        ...
        findRootNode : function alfresco_search_Search__findRootNode(storeType, storeId, nodeModuleId)
        {
            ...
            if (NodeService.exists(storeRef))
            {
                ...
                require([ nodeModuleId || 'alfresco/node/ScriptNode' ], function alfresco_search_Search__findNode_toScript(NodeModule)
                {
        ...
        /**
         * Looks up a single node by its reference and returns a script representation of it. The equivalent operation in the Rhino-based
         * class Search also supports a web script-friendly lookup of patterns like "{store_type}/{store_id}/{node_id}" or
         * "{store_type}/{store_id}/{path}". This variant is not supported in this module to discourage bad practice and avoid creep of
         * utility-type functionality into a core module. This module does however provide [findRootNode]{@link module:alfresco/search/Search#findRootNode}
         ...
         * @param {string}
         *            [nodeModuleId] the name of the script module to use for representing the node (defaults to [alfresco/node/ScriptNode]{@link module:alfresco/node/ScriptNode})
        ...
        findNode : function alfresco_search_Search__findNode(ref, nodeModuleId)
        {
            ...
            if (NodeService.exists(nodeRef) && PermissionService.hasPermission(nodeRef, PermissionServiceAPI.READ) === AccessStatus.ALLOWED)
            {
                require([ nodeModuleId || 'alfresco/node/ScriptNode' ], function alfresco_search_Search__findNode_toScript(NodeModule)
                {
                    result = new NodeModule(nodeRef);
